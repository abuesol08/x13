...
c     ------------------------------------------------------------------
     ...
     &          igrptl*(PGRPCR),str*(12),perstr*(7)
      DOUBLE PRECISION Sscut,Maxsrs,Minsrs,tmp,divfac
      LOGICAL allmss,argok,Readok,Lchkin,Lx11,Lmodel,Lcomp,prt,prterr,
     &        Hvmfil,Lexist,Lexout,Lexerr,lexsum,sav,Ldata,Lfrtop,
     &        prtwrn,Lgraf,Lexgrf,lerr,lzero,tdfneg,alltdf,Lseats,lsadj
      INTEGER ktd,Notc,i,itst,iyrs,nsp,j,Nsrscr,ip0,ip1,ip2,rgmgrp,
     &        nyr,Fhandl,nchr,igrp,ipos,begcol,endcol,ndtchr,iper,
...
c     ------------------------------------------------------------------
      CHARACTER XAICDC*28
      INTEGER xaicpt,PXTAIC
      PARAMETER(PXTAIC=6)
      DIMENSION xaicpt(0:PXTAIC)
...
c-----------------------------------------------------------------------
     ...
     &    .and.Ncusrx.gt.0)THEN
       ...
       DO WHILE (i.le.Ncusrx)
        IF(Usrtyp(i).eq.PRGTTD.and.ktd.eq.0)ktd=-i
        IF(Usrtyp(i).eq.PRGTLM.and.klm.eq.0)klm=-i
        IF(Usrtyp(i).eq.PRGTLQ.and.klq.eq.0)klq=-i
        IF(Usrtyp(i).eq.PRGTLY.and.kly.eq.0)kly=-i
        IF(Usrtyp(i).eq.PRGTST.and.kstd.eq.0)kstd=-i
        i=i+1
       END DO
      END IF
...
c-----------------------------------------------------------------------
     ...
     &          Rgvrtp(begcol).eq.PRGTLY).AND.(Itdtst.eq.1.or.
     &          Itdtst.eq.4))THEN
...
c     ------------------------------------------------------------------
      IF(Leastr.and.Neasvc.eq.4)THEN
...
c-----------------------------------------------------------------------
        IF(Nusxrg.gt.0)THEN
         iusr=1
         DO icol=1,Nbx
          IF(Rgxvtp(icol).eq.PRGTUD.and.Nusxrg.gt.0)THEN
           rtype=Usxtyp(iusr)
           iusr=iusr+1
           IF(Tdgrp.eq.0.and.rtype.eq.PRGTTD)THEN
            Tdgrp=icol
            IF(Xtdtst.gt.0)THEN
             Xtdtst=0
             Xuser=T
            END IF
           ELSE IF(Stdgrp.eq.0.and.rtype.eq.PRGTST)THEN
            Stdgrp=icol
           END IF
          ELSE IF(Rgxvtp(icol).eq.PRGTUH.and.(.not.(Holgrp.gt.0)))THEN
           Holgrp=icol
...
c-----------------------------------------------------------------------
     ...
     &       rtype.eq.PRG1TD.or.rtype.eq.PRR1TD.or.rtype.eq.PRA1TD)
     &       Nflwtd=Nflwtd+1
          IF(rtype.eq.PRGTLM.or.rtype.eq.PRGTLQ.or.
     &       rtype.eq.PRRTLM.or.rtype.eq.PRRTLQ.or.
     &       rtype.eq.PRATLM.or.rtype.eq.PRATLQ)
     &       Nln=Nln+1
          IF(rtype.eq.PRGTSL.or.rtype.eq.PRRTSL.or.rtype.eq.PRATSL)
     &       Nsln=Nsln+1
          IF(rtype.eq.PRGTLY.or.rtype.eq.PRRTLY.or.rtype.eq.PRATLY)
     &       Nlp=Nlp+1
         END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
         IF(rtype.eq.PRGTAO.or.rtype.eq.PRSQAO.or.rtype.eq.PRGTMV)
     &      Nao=Nao+1
c-----------------------------------------------------------------------
c     regARIMA Level Change Outlier regressors
c-----------------------------------------------------------------------
         IF(rtype.eq.PRGTLS.or.rtype.eq.PRGTRP.or.rtype.eq.PRGTTL.or.
     &      rtype.eq.PRGTQI.or.rtype.eq.PRGTQD.or.rtype.eq.PRSQLS)THEN
          Nls=Nls+1
...
C-----------------------------------------------------------------------
       IF(Readok.and.Lgraf)THEN
        ...
        END IF
 1062   FORMAT(/,'  NOTE: The ',a,' ',a,' file (.udg) ',
     &           'has been stored',
     &           'stored',
