...
c-----------------------------------------------------------------------
      ...
      PARAMETER(SETDIC='StochasticRegressionTotalRevision error of')
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      ELSE IF(IndxCode.ge.1035.and.IndxCode.le.1044)THEN
       iss=IndxCode-1034
       i2=iss/2 + 1
       IF(i2.eq.4.and.Ny.eq.4)THEN
        CALL getstr(SSEDIC,sseptr,PSSE,PSSE,eststr,nstr)
       ...
       IF(i2.eq.1)THEN
        CALL makeIndexLink(Fh,Indx,CsrsHTML(n1:NcsHTML)//'.html',
     &                     'Sliding spans Histogram for '//
     &                     eststr(1:nstr),T,F)
       ELSE
        CALL makeIndexLink(Fh,Indx,CsrsHTML(n1:NcsHTML)//'.html',
     &                     'Sliding spans Histogram for '//
     &                     eststr(1:nstr)//' (indirect)',T,F)
       END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &                     'Long term trend Component',T,F)
       ELSE
        ...
        IF(iss.gt.8)THEN
         CALL makeIndexLink(Fh,Indx,CsrsHTML(n1:NcsHTML)//'.html',
     &                 eststr(1:nstr-1)//' long term trend factors',T,F)
        ELSE IF(iss.gt.4)THEN
         CALL makeIndexLink(Fh,Indx,CsrsHTML(n1:NcsHTML)//'.html',
     &                   eststr(1:nstr-1)//' long term trend component',
     &                      T,F)
        ELSE
         CALL makeIndexLink(Fh,Indx,CsrsHTML(n1:NcsHTML)//'.html',
     &                      eststr(1:nstr-1)//' long term trend',T,F)
        END IF
       END IF
...
c-----------------------------------------------------------------------
      ELSE IF(IndxCode.gt.3000)THEN
       ...
       CALL itoc(npass,savstr,ipos)
       CALL makeIndexLink(Fh,Indx,CsrsHTML(n1:NcsHTML)//'.html',
     &                    'Outlier identification: Backward addition '//
