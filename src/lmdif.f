...
C
C     **********
      SUBROUTINE lmdif(fcn,M,N,X,Fvec,Lauto,Ftol,Xtol,Gtol,Mxiter,
     &                 Epsfcn,Diag,Mode,Factor,Nprint,Info,Nliter,Nfev,
...
C
       iflag=1
       CALL fcn(M,N,X,Fvec,Lauto,iflag,F)
c      NFEV=1
       Nfev=Nfev+1
       IF(iflag.ge.0)THEN
        fnorm=enorm(M,Fvec)
...
C
         iflag=2
         CALL fdjac2(fcn,M,N,X,Fvec,Lauto,Fjac,Ldfjac,iflag,Epsfcn,Wa4,
     &               F)
         Nfev=Nfev+N
c-----------------------------------------------------------------------
...
*         write(ng,9004)' within lmdif (4):',gnorm,Gtol
...
C
          iflag=1
          CALL fcn(M,N,Wa2,Wa4,Lauto,iflag,T)
          Nfev=Nfev+1
          IF(iflag.lt.0)GO TO 20
          fnorm1=enorm(M,Wa4)
C
...
*          write(ng,9001)' within lmdif (1):',dabs(actred),Ftol,prered,
*     &                  Ftol,P5,ratio,ONE
          IF(delta.le.Xtol*xnorm)Info=2
*          write(ng,9002)' within lmdif (2):',delta,Xtol,xnorm
          IF(dabs(actred).le.Ftol.and.prered.le.Ftol.and.
...
*          write(ng,9005)' within lmdif (5a):',Mxiter,Nliter
          IF(Nfev-oldfev.ge.maxfev)Info=5
*          write(ng,9006)' within lmdif (5b):',Nfev,oldfev,maxfev
          IF(dabs(actred).le.epsmch.and.prered.le.epsmch.and.
     &       P5*ratio.le.ONE)Info=6
*          write(ng,9007)' within lmdif (6):',dabs(actred),epsmch,
*     &                  prered,epsmch,P5,ratio,ONE
          IF(delta.le.epsmch*xnorm)Info=7
*          write(ng,9008)' within lmdif (7):',delta,epsmch,xnorm
          IF(gnorm.le.epsmch)Info=8
*          write(ng,9009)' within lmdif (8):',gnorm,epsmch
*          write(ng,9000)
*          write(ng,8000)Nliter,dabs(actred),prered,Ftol
          IF(Info.ne.0)GO TO 20
C
