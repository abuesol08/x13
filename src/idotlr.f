...
c-----------------------------------------------------------------------
     ...
     &          hdrstr*(15),savstr*(100),calmst*(105)
      LOGICAL lalmst,Lestim,locok,lprthd,Ltstao,Ltstls,Ltsttc,
     ...
     &        mxcode,addnum,ibgls,ibgir,nlstst,ntctst,nirtst
...
c        oldotl=oldotl+1
c-----------------------------------------------------------------------
c     Also, if automatic model idenfication is used, check how many
c     automatic outliers are in regression model and omit .
c-----------------------------------------------------------------------
       ELSE IF(Lauto.and.(Rgvrtp(icol).eq.PRGTAA.or.
...
c-----------------------------------------------------------------------
           IF(.not.Lhiddn)WRITE(Mt1,1050)tmpttl(1:ntmpcr)
           CALL errhdr
...
c-----------------------------------------------------------------------
            IF(.not.Ladd1)THEN
             IF(.not.lalmst)THEN
              ...
              IF(Sviter)THEN
               CALL svolit(LWRITE,ipass,'+',tmpttl,ntmpcr,
...
c-----------------------------------------------------------------------
       IF(lalmst)THEN
        IF(.not.Lhiddn.and..not.lprthd)WRITE(Mt1,1080)cdash(1:ldash(n0))
 1080   FORMAT(' ',a,//)
        GO TO 50
       END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
        IF(Sviter)THEN
         CALL svolit(LWRITE,ipass,'+',mxotl,nmxocr,mxotlt/rbmse,
...
c-----------------------------------------------------------------------
       iptr=1
       delno=T
       DO WHILE(iptr.le.POTLR.and.minptr(iptr).ne.NOTSET.and.delno)
        iptr2=minptr(iptr)
...
c-----------------------------------------------------------------------
         IF(Sviter)THEN
          CALL svolit(LWRITE,ipass,'-',tmpttl,ntmpcr,mint(iptr2),ZERO,
     &                sqrt(Var*Nefobs/(Nefobs-Ncxy+1)),Sviter,Lxreg)
          IF(Lfatal)RETURN
         END IF
         IF(Ldiag)THEN
          CALL svolit(LWRITE,ipass,'-',tmpttl,ntmpcr,mint(iptr2),ZERO,
     &                sqrt(Var*Nefobs/(Nefobs-Ncxy+1)),F,Lxreg)
          IF(Lfatal)RETURN
         END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       IF(Sviter)
     &    CALL svolit(LCLOSE,ipass,'*',tmpttl,1,ZERO,ZERO,ZERO,Sviter,
     &                Lxreg)
       IF(Ldiag)THEN
