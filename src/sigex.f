C     Last change:      REG  05 Jun 2006, 09 May 2006, 24 Apr 2006
...
C 
     ...
     $           PHIm,nPHIm,THETm,nTHETm,Vm,WithoutVf)
        if (ilam.eq.1) then
         do i=1,NZ+LFor
          eTrend(i)=Trend(i)+PaOutR(i)+Pareg(i,1)
         enddo
        else
         do i=1,NZ+LFor
          eTrend(i)=log(Trend(i))+log(PaOuTR(i))+log(Pareg(i,1))
         enddo
        end if
...
c         call UsrEntry(HPtrend,nz+1,nz+lFor,1,MPKP,1331)
     ...
     $            PHIm,nPHIm,THETm,nTHETm,Vm,WithoutVf)
        if (ilam.eq.1) then
         do i=1,NZ+LFor
          extSA(i)=SA(i)+PaOuTR(i)+Pareg(i,1)+Pareg(i,3)
     $                  +Pareg(i,4)+Pareg(i,7)
c             if (MQ.le.2) then
          extSA(i)=extSA(i)+PaOuIR(i)
c            end if
          if (.not.isCloseToTD) then
           extSA(i)=extSA(i)+Pareg(i,5)
          end if
         enddo
        else
         do i=1,NZ+LFor
...
C Calcularemos el Business cycle de la serie interpolada (Tram)
     ...
     $             PHIm,nPHIm,THETm,nTHETm,Vm,withoutVf)
        if (iLAM.eq.1) then
         do i=1,NZ+lFor
          extZ(i)=Z(i)+PaouTR(i)+Pareg(i,3)+Pareg(i,4)
     ...
     $                  +Pareg(i,1)
         ...
         do i=1,NZ+lFor
          extZ(i)=Z(i)+log(PaouTR(i))+log(Pareg(i,3))
