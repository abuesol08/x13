C     Last change:  BCM  23 Mar 2005    3:41 pm
      SUBROUTINE x11ari(Lmodel,Lx11,X11agr,Lseats,Lcomp,Issap,Irev,
     &                  Irevsa,Ixreg,Lsumm,Lgraf)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &        Lseats,lfm,Lgraf,Rvxotl,X11agr,lsadj,Lsvtpk
...
c-----------------------------------------------------------------------
     ...
     &                Lrfrsh,Revfix,Revfxx,Rvtran,Cnctar,Rvxotl
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF(Lsumm.gt.0.and.Lx11)THEN
       CALL cpu_time(ticks)
*       WRITE(*,9000) 'bx11:',ticks
       IF(Issap.lt.2.and.Irev.lt.4)THEN
...
*       write(*,*)'  enter genqs'
     ...
     &             Ny,Svltab(LSLQS).or.Svltab(LSLDQS),T)
       END IF
       IF(Ny.eq.12)THEN
        IF(Lsumm.gt.0)THEN
         CALL cpu_time(ticks)
*         WRITE(*,9000) 'bspectrum:',ticks
...
*       write(*,*)'  enter prtkup'
     ...
     &                  Svltab(LSLTPK).or.Svltab(LSLDTP))
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       IF(Iagr.eq.3)THEN
        ...
        IF(X11agr)THEN
         ...
         IF(.not.Lfatal)CALL x11pt4(Lgraf)
        ELSE
     ...
     &               Iagr,Ny,Svltab(LSLQS).or.Svltab(LSLIQS),
     ...
     &        CALL svtukp(Iagr,Lsumm,cstuk,cttuk,cstk90,cttk90,lsadj,
     &                  Svltab(LSLTPK).or.Svltab(LSLITP))
          END IF
         END IF
        END IF
        IF(Lfatal)RETURN
