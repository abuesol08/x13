C     Last change:  BCM  14 Oct 1998    4:01 pm
      SUBROUTINE getchk(Mxcklg,Acflim,Iqtype,Sp,Inptok)
c     ------------------------------------------------------------------
      IMPLICIT NONE
c     ------------------------------------------------------------------
      INCLUDE 'lex.i'
...
c     ------------------------------------------------------------------
      ...
      PARAMETER(ZERO=0.0D0,F=.false.,T=.true.)
c     ------------------------------------------------------------------
      DOUBLE PRECISION Acflim,dvec
...
c-----------------------------------------------------------------------
      ...
      PARAMETER(ARGDIC='maxlagprintsavesavelogacflimitqtype')
c     ------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      DATA argptr/1,7,12,16,23,31,36/
...
c-----------------------------------------------------------------------
      DO WHILE (T)
       IF(gtarg(ARGDIC,argptr,PARG,argidx,arglog,argok))THEN
        IF(Lfatal)RETURN
        GO TO(10,20,30,40,50,60),argidx
c-----------------------------------------------------------------------
