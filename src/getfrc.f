     ...
     &                  Sp,Lindfr,Lfctfr,Inptok)
      IMPLICIT NONE
c-----------------------------------------------------------------------
c     Get the seasonal adjustment options for X-13ARIMA-SEATS.
c----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
   30   CALL gtdcvc(LPAREN,T,1,FRTDIC,frtptr,PFRT,
     ...
     &              ivec,nelt,argok,Inptok)
        IF(Lfatal)RETURN
        IF(.not.argok)
     &     CALL writln('        permprioradj, or both.',STDERR,Mt2,F)
   ...
   40   CALL gtdcvc(LPAREN,T,1,SUMDIC,sumptr,PSUM,
     ...
     &                'No seasonal period specified in series spec.')
         ...
         ELSE 
          ...
          ELSE
           IF(Sp.eq.12)THEN
            CALL inpter(PERROR,Errpos,'This entry for start only valid f
     &or monthly data.')
           ELSE 
            CALL inpter(PERROR,Errpos,'This entry for start only valid f
     &or quarterly data.')
...
c-----------------------------------------------------------------------
        IF(argok.and.nelt.gt.0)THEN
         IF(dvec(1).lt.ZERO.or.dvec(1).gt.ONE.or.dpeq(dvec(1),ZERO))THEN
          CALL inpter(PERROR,Errpos,'Value of rho must be greater than 0
...
c-----------------------------------------------------------------------
  110   CALL gtdcvc(LPAREN,T,1,FMDDIC,fmdptr,PFMD,
     &              'Available options for model are ratio or diff.',
     &              ivec,nelt,argok,Inptok)
        IF(Lfatal)RETURN
