...
c-----------------------------------------------------------------------
     ...
     &          dpeq(QSirrS2,DNOTST).and.dpeq(QSoriS2,DNOTST))
      IF(Prttab(LSPCQS).and.(lqs.or.lqss))THEN
       IF(lqs)THEN
        IF(Lorig)THEN
...
c-----------------------------------------------------------------------
     ...
     &    WRITE(Nform,1030)'qsrsd',QSrsd,chisq(QSrsd,2)
       ...
       END IF
      END IF
      IF(Savtab(LSPCQS).and.lqss)THEN
       IF(.not.dpeq(QSoriS,DNOTST).and.Iagr.lt.4)
