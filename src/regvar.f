...
c     ------------------------------------------------------------------
     ...
     &        Reglom,smpday,strinx,idtpos,regmdt,itype,thisgp,rtype2
      DOUBLE PRECISION lomadj,Userx,Y,tsrs,emean
      DIMENSION Bgusrx(2),Begxy(2),Userx(PUSERX),Y(Nobpf),regmdt(2),
     &          tsrs(PLEN),begrgm(PLEN),emean(PSP)
      EXTERNAL chkcvr,ctoi,strinx
c----------------------------------------------------------------------
      CHARACTER UTYDIC*153
      INTEGER utyidx,utyptr,PUTY
      PARAMETER(PUTY=6)
     ...
     &day Group 4User-defined Holiday Group 5')
c----------------------------------------------------------------------
      DATA utyptr / 1,22,42,70,98,126,154 /
c----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
      IF(Nrxy*Ncxy.gt.PXY)THEN
       CALL errhdr
       WRITE(STDERR,1010)Nrxy,Ncxy,PXY
       WRITE(Mt2,1010)Nrxy,Ncxy,PXY
 1010  FORMAT(/,' Too many elements in [X:y]',i4,' *',i3,' >',i6)
       CALL abend
       RETURN
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &       140,140,140,120,120,140),rtype2
c-----------------------------------------------------------------------
...
c* Use this for regARIMA instead, X-13A-S uses leap year as a base
c*   80  CALL td7var(Begxy,Sp,Nrxy,Ncxy,begcol,F,T,F,Xy,begrgm)
c-----------------------------------------------------------------------
