...
c-----------------------------------------------------------------------
      ...
      DOUBLE PRECISION sumb,sumcr,sumcr2,tdwrg,tdwcr,tdwcr2,addon
      INTEGER icol,imark,nrg,ncr,ncr2,ncol,ncol2,ncol3,igrp,begcol,
     &        endcol
      DIMENSION tdwrg(6),tdwcr(6),tdwcr2(6)
c-----------------------------------------------------------------------
      LOGICAL dpeq
      EXTERNAL dpeq
c-----------------------------------------------------------------------
...
c     ------------------------------------------------------------------
     ...
     & '---------------------------------------------------------------'
      ...
      IF(ncr2.gt.0)THEN
       ...
       IF(dpeq(Lam,ZERO))THEN
        WRITE(Mt1,1030)(tdwcr2(icol), icol = 1, 6), sumb
       ELSE
        WRITE(Mt1,1040)(tdwcr2(icol), icol = 1, 6), sumb
       END IF
      END IF
      WRITE(Mt1,1000)
     & '---------------------------------------------------------------'
