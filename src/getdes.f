...
c     ------------------------------------------------------------------
      ...
      ELSE IF(Itbl.le.PDSUM8)THEN
       CALL makttl(DSCDIC,dscptr,PDSC,Itbl,PDSUM7,Fildes,Ndescr,Label,F)
      ELSE IF(Itbl.le.PDSUM9)THEN
       CALL makttl(DC2DIC,dc2ptr,PDC2,Itbl,PDSUM8,Fildes,Ndescr,Label,F)
      ELSE
       CALL makttl(DSEDIC,dseptr,PDSE,Itbl,PDSUM9,Fildes,Ndescr,Label,F)
      END IF
c     ------------------------------------------------------------------
      RETURN
      END
