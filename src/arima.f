C     Last change:  SRD  25 Jan 100    1:30 pm
...
c     ------------------------------------------------------------------
     ...
     &        Lgraf,gudrun,ldiag,ltmp,lauto,lsadj,ltdlom
...
C-----------------------------------------------------------------------
     ...
     &                Begtst,Endtst,nefobs,Lestim,Mxiter,Mxnlit,lester,
...
c-----------------------------------------------------------------------
     ...
     &             Lsrun,Prttab(LESTES),Prttab(LOTLTL),
     &             Prttab(LESTIT))
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
       IF(Prttab(LESTRT).or.Savtab(LESTRT))THEN
        CALL prtrts(Prttab(LESTRT),Savtab(LESTRT))
        IF(Lfatal)RETURN
       END IF
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
          CALL pracf2(nefobs,a,na,Mxcklg,Lgraf)
          IF(Lfatal)RETURN
         END IF
c-----------------------------------------------------------------------
         IF(Prttab(LCKHST).and.Var.gt.ZERO)THEN
          WRITE(Mt1,1170)
          CALL hist(a(na-nefobs+1),Begspn,Sp,nefobs,Nobs-nefobs)
          IF(Lfatal)RETURN
         END IF
c-----------------------------------------------------------------------
