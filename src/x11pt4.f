...
c-----------------------------------------------------------------------
     ...
     &          TWO=2D0,SIX=6D0)
c-----------------------------------------------------------------------
      LOGICAL lsame,Lgraf,pre18b,gudbak,allgud
...
c     ------------------------------------------------------------------
     ...
     &   Iagr.lt.4).or.(Iagr.eq.4.and.Lindls))THEN
...
c-----------------------------------------------------------------------
     ...
     &    CALL fgen(Mt1,Kfmt,Prttab(LCMPF2),Prttab(LCMPF3))
       CALL svf2f3(Nform,Ng,Savtab(LCMPF2),Savtab(LCMPF3),'if')
       IF(Lfatal)RETURN
      ELSE
       IF(Prttab(LX11F2).or.Prttab(LX11F3))
     &    CALL fgen(Mt1,Kfmt,Prttab(LX11F2),Prttab(LX11F3))
