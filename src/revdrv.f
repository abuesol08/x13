...
c-----------------------------------------------------------------------
      CHARACTER outstr*(85),igrptl*(PGRPCR),datstr*(10),cmdl*(132),
     ...
     &               Grpttx,Grpx,Gpxptr,Ngrptx,1,.not.lstxfx)
       ...
       IF(Ncusrx.gt.0)THEN
        ...
        IF(upuser)THEN
         ...
         CALL bakusr(Userx,Usrtyp,Usrptr,Ncusrx,Usrttl,Regfx,B,
     &               Grpttl,Grp,Grpptr,Ngrptl,0,.not.lastfx)
         CALL ssprep(T,F,F)
        END IF 
       END IF 
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
*       CALL timer(itick1)
*       WRITE(Ng,*)  '  timer(1) = ', itick1, ' (time up to x11ari)'
       CALL x11ari(Lmodel,Lx11,X11agr,Lseats,Lcomp,Issap,Irev,Irevsa,
     &             Ixreg,0,F)
       IF(Lfatal)RETURN
*       CALL timer(itick2)
*       WRITE(Ng,*)  '  timer(2) = ', itick2, ' (time after x11ari)',
*     &              '  timer(2) - timer(1) = ', itick2 - itick1
c-----------------------------------------------------------------------
       IF(Irev.lt.0)THEN
        Irev=0
...
c-----------------------------------------------------------------------
     ...
     &               cmdl,ncmdl)
         IF(Lfatal)RETURN
         IF(Prttab(LREVR9))WRITE(Mt1,2110)datstr(1:ndate),cmdl(1:ncmdl)
