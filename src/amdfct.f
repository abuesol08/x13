...
c-----------------------------------------------------------------------
      IF(.not.Bckcst.and.(((nobsot-(Mxdflg+Mxarlg))*Ncxy)+1).le.0)THEN
       IF(.not.Lauto)CALL writln('NOTE: Insufficient data to compute ave
     &rage forecast error diagnostic.',Mt1,Mt2,T)
       Fctok=F
       RETURN
      END IF
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &      rtype.eq.PRGTLS.or.rtype.eq.PRGTAL.or.rtype.eq.PRGTRP.or.
     &      rtype.eq.PRGTTC.or.rtype.eq.PRGTAT.or.rtype.eq.PRGTSO.or.
*     &      rtype.eq.PRGTAS.or.rtype.eq.PRGTTL)THEN
