...
c        
      if ((tabtables .eq. d_tabtables) .and. (hpcycle .eq. 1)) then
        if ((NumSer .gt. 250) .and. (NumSer .le. 1000)) then
...
C
        ...
        if ((Iter .eq. 0).and. (.not.matopened)) then
          nouts=ISTRLEN(Outdir)
          filename=' '
          if (nouts.gt.0) THEN
             filename=Outdir(1:ISTRLEN(Outdir)) // '\\summarys.txt'
          else
...
c             call OPENDEVICE (filename,65,0,ifail)
          if (ifail.gt.0) THEN
            Ierr=ifail
            Errext='Error opening summarys.txt'
            RETURN
          END IF
c          call Mtx1Reset()
c          call Mtx2Reset()
            write (65,'(3x,A)') mattitle(1:22)
            write (65, '(3x,''NZ ='',I3.3,'';'',3x,''PERIOD='',
     $               I2.2,''-''I4.4,''/'',I2.2,''-'',I4.4,'';'',
     $               3x,''MQ=''I2.2,'';'',/)')
     $                Nz,Nper,Nyer,
...
C   LINES OF CODE ADDED FOR X-13A-S : 5
            if (noutdir.gt.0) THEN
              filename = Outdir(1:ISTRLEN(Outdir)) // '\\sgeneral.m'
            else
...
c          call Mtx1Reset()
            write (65 ,'(3x,''n'',3x,''Title'',17x,''Pread.'',x,
     $            ''Model'',3x,
     $            ''Approx.'',15x,''Model'',20x,''SD(a)'',4x,
     $            ''SEAS_NP(a)'',2x,''Spectr.'',x,''Check'',2x,
     $            ''Check'',5x,''Determ.'')')
            write (65,'(36x,''Changed'',x,''to NA'',63x,''Factor'',
     $             2x,''on ACF'',x,''on CCF'',2x,''Comp. Modif.'')')
            write (65 ,'(53x,''m'',4x,''p'',4x,''d'',4x,
     $            ''q'',4x,''bp'',4x,''bd'',4x,''bq'',
     $            48x,''TC'',x,''S'',x,''U'',x,
     $            ''Trans'',x,''SA'')')
*          end if 
C   LINES OF CODE ADDED FOR X-13A-S : 5
            if (noutdir.gt.0) THEN
              filename = Outdir(1:ISTRLEN(Outdir)) // 'sparami.m'
            else
...
C   LINES OF CODE ADDED FOR X-13A-S : 5
            if (noutdir.gt.0) THEN
              filename = Outdir(1:ISTRLEN(Outdir)) // 'sparamii.m'
            else
...
C   END OF CODE BLOCK
            ...
            write (66 ,'(3x,''n'',3x,''Title'',43x,''SD(innov)'',36x,
     $           ''SE Est.'',16x,''SE Rev.'',21x,
     $           ''SE : Rates of Growth'')')
            write (66,'(100x,''(Conc.)'',16x,''(Conc.)'',
     $                     16x,''SE T11'',19x,''SE T1Mq'')')
            write (66,'(142x,''(One Period)'',11x,
     $           ''(Annual Centered)'')')
            write (66 ,'(33x,''TC'',9x,''S'',5x,''Trans'',5x,''StocTD'',
     $           8x,''U'',8x,''SA'',11x,''TC'',8x,''SA'',11x,''TC'',8x,
     $           ''SA'',12x,''TC'',8x,''SA'',9x,''X'',8x,''TC'',
     $            8x,''SA'')')
            write (67 ,'(3x,''n'',3x,''Title'',36x,''Convergence'',
     $           23x,''Signif.Stoch.'',21x,''DAA'')')
            write (67,'(50x,''(in %)'',26x,''Season. (95%)'')')
            write (67,'(42x,''1Y'',17x,''5Y'')')
            write (67 ,'(37x,''TC'',8x,''SA'',8x,''TC'',8x,''SA'',
...
C   LINES OF CODE ADDED FOR X-13A-S : 5
          if (noutdir.gt.0) THEN
              filename = Outdir(1:ISTRLEN(Outdir)) // '\\ROGTABLE.OUT'
          else
...
C   LINES OF CODE ADDED FOR X-13A-S : 5
          if (noutdir.gt.0) THEN
            filename = Outdir(1:ISTRLEN(Outdir)) // 'TABLE-S.OUT'
          else
...
*            call profiler(2,'before NMOut')
            ...
            write (65,'(i4,a,x,a,3x,a,6x,a,8x,a,3x,i2,3x,i2,3x,i2,3x,
...
*            call profiler(2,'before OutNoPar')
            ...
            write(66,
     $          '(i4,a,x,a,5(x,f9.0),2(4x,f9.0,x,f9.0),5(x,f9.0))')
     $       niter, errch, mattitle(1:22), DONE, DONE, DONE, DONE, DONE,
     $       DONE, DONE,DONE,DONE,DONE, DONE, DONE, DONE, DONE
            write (67,'(i4,a,x,a,x,f9.0,x,f9.0,x,f9.0,x,f9.0,11x,i2,
     $              8x,i2,8x,i2,4x,f9.0,x,f9.0)')
...
*                  call profiler(2,'before wrHeadTGenSumS')
                  ...
                  write (65, '(7x,A,6x,A,8x,A,4x,I1,4x,I1,4x,I1,
     $            4x,I1,5x,I1,5x,I1,5x,I1,2x,g11.4,5x,g11.4,5x,A,7x,A,
     $            7x,A,2x,A,x,A,x,A,5x,A,2x,A)')
     $            getPat(),getTmcs(),
     ...
     $            getCmtIR(),getCmtTs(),getCmtSA()
                  ...
                  write (65,'(3x,''Decomposition : Standard Errors'')')
                  write (65,*)
                  write (65 ,'(26x,''SD(innov)'',28x,
     $            ''SE Est.'',16x,''SE Rev.'')')
                  write (65,'(63x,''(Conc.)'',16x,''(Conc.)'')')
                  write (65 ,'(8x,''TC'',9x,''S'',5x,A6,8x,
     $           ''U'',8x,''SA'',11x,''TC'',8x,''SA'',11x,''TC'',8x,
     $            ''SA'')') auxS
                  write (65, '(5x,g11.4,1x,g11.4,1x,g11.4,1x,g11.4,1x,
     ...
     $           getRseCect(),getRseCecSa()
                  write (65,*)
                  write (65 ,'(14x,''SE : Rates of Growth'')')
                  write (65,'( 9x,''SE T11'',19x,''SE T1Mq'')')
                  write (65,'(6x,''(One Period)'',11x,
     $             ''(Annual Centered)'')')   
                  write (65 ,'(8x,''TC'',8x,''SA'',9x,''X'',8x,''TC'',
     $             8x,''SA'')')
                  write (65, '(x,f9.2,x,f9.2,x,f9.2,x,f9.2,x,f9.2)')
     $           getT11t(),getT11Sa(),getT112x(),getT112t(),getT112Sa()
                  write (65,*)
                  write (65,*)
c
...
c   escribimos los modelos de los componentes
                  ...
                  write(65,*)
                  write(65,*)
                  write (65,'(x,"Model for the components:")') 
                  write(65,*)
                  write(65,*)
                  if (lu61.ne.' ') then
                    write (65,'(2x,"Trend-cycle:")') 
                    write (65,'(2x,A)') lu61(1:istrlen(lu61))
                    write(65,*)
                  end if
                  if (lu62.ne.' ') then
                    write(65,'(2x,"Seasonal:")')
                    write(65,'(2x,A)') lu62(1:istrlen(lu62))
                    write(65,*)
                  end if
                  if (lu63.ne.' ') then
                    write(65,'(2x,"SA series:")') 
                    write(65,'(2x,A)') lu63(1:istrlen(lu63))
                    write(65,*)
                  end if
                  if (lu64.ne.' ') then
                    if (IsCloseToTD) then
                      write (65,'(2x,"TD stoch.:")')
                    else 
                      write (65,'(2x,"Transitory:")') 
                    end if
                    write (65,'(2x,A)') lu64(1:istrlen(lu64))
                    write(65,*)
                  end if
                  if (lu64I.ne.' ') then
                    write (65,'(2x,"Irregular:")') 
                    write (65,'(2x,A)') lu64I
                  end if
              else if ((matopened) .and. (Iter .gt. 0)) then
                write (65,'(i4,3x,a,3x,a,6x,a,8x,a,4x,i1,4x,i1,4x,i1,
     ...
     $            getSf(),getCvar(),getCcc(),getCmtTc(),getCmtS(),
     $            getCmtIR(),getCmtTs(),getCmtSA()
                write (66, '(i4,3x,a,6(x,g11.4),4x,g11.4,x,g11.4,4x,
     $            g11.4,x,g11.4,x,f9.2,x,f9.2,5x,
     $            g9.2,x,g9.2,x,g9.2)')niter,mattitle(1:22),
     $            getSdt(),getSds(),
     ...
     $            getT112x(),getT112t(),getT112Sa()
                write (67, '(i4,3x,a,1x,f9.1,1x,
     $            f9.1,1x,f9.1,1x,f9.1,11x,I2,8x,I2,8x,I2,
     $            4x,f9.2,1x,f9.2)')niter,mattitle(1:22),
     $            getCovt1(),getCovSa1(),
     $            getCovt5(),getCovSa5(),
     $            getSsh(),getSsp2(),getSsp2(),
     $            getDaat(),getDaaSa()
...
*      call profiler(2,'GO TO 5119, line 3153')
      goto 5119
 5017 continue
      write (Nio,'(4X,''NOT ENOUGHT OBSERVATIONS'')')
     ...
     $         'PLEASE CHECK SERIES LENGTH', 'FOR THE SERIES :'
      ...
      if ((matopened) .and. (iter .gt. 0)) then
        ...
        call MTX1RESET
        call MTX2RESET
        call ErrorLog('NOT ENOUGHT OBSERVATIONS',1)
        write (65,
     $'(i4,''$'',2x,a,3x,a,6x,a,8x,a,3x,i2,3x,i2,3x,i2,3x,i2,4x,i2,4x,
     $        i2,4x,i2,2x,f9.0,2x,f9.0,5x,a,7x,a,7x,a,2x,a,x,a,x,
     $        a,5x,a,2x,a)')
...
c	      call OutNoPar(74,niter,mattitle)
     ...
     $          qstat,wm,0)
        write (66,'(i4,''$'',2x,a,x,f9.0,x,f9.0,x,f9.0,x,f9.0,
     $           x,f9.0,4x,f9.0,
     $           x,f9.0,4x,f9.0,x,f9.0,x,f9.0,x,f9.0,x,f9.0,x,
     $           f9.0,x,f9.0)')
     ...
     $       DONE, DONE, DONE, DONE, DONE
        write (67,'(
     $              i4,''$'',2x,a,x,f9.0,x,f9.0,x,f9.0,x,f9.0,11x,i2,
     $              8x,i2,8x,i2,4x,f9.0,x,f9.0
     $           )')
     $       niter, mattitle(1:22),DONE, 
     $       DONE, DONE,
...
c          call MTX2RESET
          ...
          write (65,
     $'(i4,''^'',2x,a,3x,a,6x,a,8x,a,4x,i1,4x,i1,4x,i1,4x,i1,5x,i1,5x,
     $        i1,5x,
     $        i1,4x,g11.4,5x,g11.4,3x,a,7x,a,6x,a,3x,a,x,a,x,a,
     $        5x,a,2x,a)')
...
c
          call OutNoPar(74,niter,mattitle)
          write (66,'(i4,''^'',2x,a,x,g11.4,x,g11.4,x,g11.4,x,g11.4,
     $           x,g11.4,x,g11.4,4x,g11.4,
     $           x,g11.4,4x,g11.4,x,g11.4,x,g11.4,x,f9.2,9x,a,9x,
     $           a,9x,a)')
...
c
          write (67,'(
     $              i4,''^'',2x,a,x,f9.1,x,f9.1,x,f9.1,x,f9.1,11x,i2,
     $              8x,i2,8x,i2,4x,f9.2,x,f9.2
     $           )')
...
c     $                  niter,mattitle(1:22)
          ...
          call wrHeadTGenSumS(65)
          write (65,'(7x,a,6x,a,8x,a,4x,i1,4x,i1,4x,i1,4x,i1,5x,i1,5x,
     $      i1,5x,i1,4x,g11.4,5x,g11.4,3x,a,7x,a,6x,a,3x,a,x,a,x,a,
     $      5x,a,2x,a)') 
     ...
     $          ''SE Est.'',16x,''SE Rev.'')')
          write (65,'(63x,''(Conc.)'',16x,''(Conc.)'')')
          write (65 ,'(8x,''TC'',9x,''S'',5x,''Trans'',9x,
     $          ''U'',8x,''SA'',11x,''TC'',8x,''SA'',11x,''TC'',8x,
     $          ''SA'')')
          write (65, '(5x,g11.4,x,g11.4,x,g11.4,x,g11.4,x,
     $          g11.4,4x,g11.4,x,g11.4,4x,g11.4,x,g11.4)')
     $         0.000,0.000, 0.000, sqf, sqf, 0.000, 0.000,0.000,0.000
          write (65,*)
          write (65 ,'(14x,''SE : Rates of Growth'')')
     ...
     $           ''(Annual Centered)'')')   
          write (65 ,'(8x,''TC'',8x,''SA'',9x,''X'',8x,''TC'',
     $           8x,''SA'')')
          write (65, '(x,f9.2,x,f9.2,9x,a,9x,a,9x,a)')
     $          0d0, 0d0, '-','-','-'
          write (65,*)
          write (65,*)
c
          call wrHeadTparIISumS(65)
          write (65,'(2x,f8.1,x,f9.1,x,f9.1,x,f9.1,11x,I2,8x,I2,8x,I2,
     $          4x,f9.2,x,f9.2)')
     $     0.0,100.0,0.0,100.0,0, 0, 0, 0.00, 0.00
          write (65,*)
          write (65,*)
          write (65,*)               
          write (65,'(3x,''Model is a pure MA. '',
...
c         
     ...
     $       'u', 'u', 'u', 'u'
           call OutNoPar(74,niter,mattitle)
           write (66,
     $'(i4,''*'',2x,a,x,f9.0,x,f9.0,x,f9.0,x,f9.0,x,f9.0,4x,f9.0,
     $           x,f9.0,4x,f9.0,x,f9.0,x,f9.0,x,f9.0,x,f9.0,x,
     $           f9.0,x,f9.0)')
