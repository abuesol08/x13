...
c-----------------------------------------------------------------------
     ...
     &       rtype.eq.PRATLM.or.rtype.eq.PRATSL.or.rtype.eq.PRATLY))
     &  .or.(rtype.eq.PRGTEA.or.rtype.eq.PRGTEC.or.rtype.eq.PRGTES.or.
     &       rtype.eq.PRGTLD.or.rtype.eq.PRGTTH).or.rtype.eq.PRGTUD)THEN
          DO icol=begcol,endcol
           IF(rtype.eq.PRGTUD)THEN
            IF((Usxtyp(iusr).eq.PRGTTD.or.Usxtyp(iusr).eq.PRGTST))THEN
             tdfx=tdfx.and.Regfxx(icol)
            ELSE IF(Usxtyp(iusr).eq.PRGTEA.or.Usxtyp(iusr).eq.PRGTLD
     &          .or.Usxtyp(iusr).eq.PRGTTH.or.Usxtyp(iusr).eq.PRGTUH
     &          .or.Usxtyp(iusr).eq.PRGTEC)THEN
             holfx=holfx.and.Regfxx(icol)
            END IF
            iusr=iusr+1
           ELSE IF(rtype.eq.PRGTEA.or.rtype.eq.PRGTEC.or.rtype.eq.PRGTES
