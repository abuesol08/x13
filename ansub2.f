C     Last change:  BCM   4 Oct 2002    2:18 pm
...
C
       ...
       if (ABS(ar(i)) .le. 1.0d0) then
        ar(i) = ACOS(ar(i))
        if (imz(i) .lt. 0.0d0) then
         ar(i) = -ar(i)
        end if
       else
        ar(i) = 0.0d0
...
c
      ...
      if (gRez(ni).gt.0.0D0) then
        gAR(ni)=(atan(gImz(ni)/gRez(ni))*180.0D0)/3.1416D0
      else if (gRez(ni).lt.0.0D0) then
        gAR(ni)=180.0D0+(atan(gImz(ni)/gRez(ni))*180.0D0)/3.1416D0
