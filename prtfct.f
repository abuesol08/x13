...
c     ------------------------------------------------------------------
       DO i=1,Nfcst
        ...
        IF(Lfatal)RETURN
        totvar=fcstse(i)**2
        stctvr=totvar-rgvar(i)
        pctrgv=100D0*rgvar(i)/totvar
        IF(Prttab(LFORVR))WRITE(Mt1,fmt)str(1:ndtchr),rgvar(i),stctvr,
     &                                  totvar,pctrgv
c     ------------------------------------------------------------------
        IF(Savtab(LFORVR))THEN
         npos=1
