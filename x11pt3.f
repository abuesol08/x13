...
c-----------------------------------------------------------------------
      CHARACTER trnchr*1
      LOGICAL Lgraf,rndok,oktrn,oktrf,gudrun,chkfct,negmsg,negfin
...
c     ------------------------------------------------------------------
     ...
     &   CALL punch(Stsie,Pos1ob,Posfob,LX11D8,F,F)
      IF((.not.Lfatal).and.Lgraf)
     &   CALL punch(Stsie,Pos1ob,Posfob,LX11D8,Lgraf,F)
      IF(Lfatal)RETURN
c     ------------------------------------------------------------------
...
c     ------------------------------------------------------------------
     ...
     &   CALL punch(Temp,Pos1ob,Posfob,LX11D9,F,F)
      IF((.not.Lfatal).and.Lgraf)
     &   CALL punch(Temp,Pos1ob,Posfob,LX11D9,Lgraf,F)
      IF(Lfatal)RETURN
...
c     ------------------------------------------------------------------
      IF(Adjao.eq.1.or.Adjtc.eq.1)THEN
       CALL copy(Sti,Posffc,1,sti2)
       IF(Adjao.eq.1)CALL addmul(sti2,Facao,sti2,Pos1bk,Posffc)
       IF(Adjtc.eq.1)CALL addmul(sti2,Factc,sti2,Pos1bk,Posffc)
