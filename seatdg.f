...
c     ------------------------------------------------------------------
     ...
     &        nstr2,Nfcst,idate,bjmdvc,Length
      ...
      CHARACTER sig*(1),cmdl*(132),ssig*(1),str1*(8),str2*(19)
...
c     ------------------------------------------------------------------
     ...
     &   Iqssm.ne.NOTSET.and.Idrsm.ne.NOTSET.and.Idssm.ne.NOTSET)THEN
       Ipssm=Ipssm/Ny
       Iqssm=Iqssm/Ny
       CALL mkmdsn(Iprsm,Idrsm,Iqrsm,Ipssm,Idssm,Iqssm,cmdl,ncmdl)
       bjmdvc(1)=Iprsm
       bjmdvc(2)=Idrsm
       bjmdvc(3)=Iqrsm
       bjmdvc(4)=Ipssm
       ...
       bjmdvc(6)=Iqssm
      ELSE
       CALL mkmdsn(P,D,Q,Bp,Bd,Bq,cmdl,ncmdl)
       bjmdvc(1)=P
       bjmdvc(2)=D
       bjmdvc(3)=Q
       bjmdvc(4)=Bp
      ...
      IF(ncmdl.gt.0)THEN
       IF(Svltab(LSLSMD))THEN
        IF(cmdl(1:ncmdl).eq.X13mdl(1:Nxmdl))THEN
         WRITE(Ng,1000)'    SEATS model',cmdl(1:ncmdl)
        ELSE
         WRITE(Ng,1000)'    SEATS model',cmdl(1:ncmdl)//
     &                 ' (SEATS routines changed ARIMA model)'
        END IF
       END IF
       IF(Lsumm.gt.0)WRITE(Nform,1000)'seatsmdl',cmdl(1:ncmdl)
       IF(cmdl(1:ncmdl).ne.X13mdl(1:Nxmdl))THEN
...
c     ------------------------------------------------------------------
      IF(Svltab(LSLTSE).or.Lsumm.gt.0)THEN
       IF(.not.dpeq(Tsetrn,DNOTST))THEN
        sig=' '
        IF(Tsetrn.gt.TSELIM)sig='*'
        IF(Svltab(LSLTSE))
     &     WRITE(Ng,1030)'    Total Squared Error (trend)',Tsetrn,sig
        IF(Lsumm.gt.0)WRITE(Nform,1030)'tsetrend',Tsetrn,sig
       END IF
       IF(.not.dpeq(Tsesea,DNOTST))THEN
        sig=' '
        IF(Tsesea.gt.TSELIM)sig='*'
        IF(Svltab(LSLTSE))
     &     WRITE(Ng,1030)'    Total Squared Error (seasonal)',Tsesea,sig
        IF(Lsumm.gt.0)WRITE(Nform,1030)'tseseasonal',Tsesea,sig
       END IF
       IF(.not.dpeq(Tsetcm,DNOTST))THEN
        sig=' '
        IF(Tsetcm.gt.TSELIM)sig='*'
        IF(Svltab(LSLTSE))
     &     WRITE(Ng,1030)'    Total Squared Error (transistory)',
     &                   Tsetcm,sig
        IF(Lsumm.gt.0)WRITE(Nform,1030)'tsetransitory',Tsetcm,sig
       END IF
       IF(.not.dpeq(Tsesad,DNOTST))THEN
        sig=' '
        IF(Tsesad.gt.TSELIM)sig='*'
        IF(Svltab(LSLTSE))
     &     WRITE(Ng,1030)'    Total Squared Error (seas adj)',Tsesad,sig
        IF(Lsumm.gt.0)WRITE(Nform,1030)'tseseasadj',Tsesad,sig
       END IF
...
c     ------------------------------------------------------------------
      IF(Svltab(LSLCVR).or.Lsumm.gt.0)THEN
       IF(.not.dpeq(Vartrn(1),DNOTST))THEN
        sig=' '
        IF(Vartrn(1).le.Vartrn(2))sig='*'
        IF(Svltab(LSLCVR))
     &     WRITE(Ng,1040)'    Trend Variance',(Vartrn(i),i=1,3),sig
        IF(Lsumm.gt.0)WRITE(Nform,1040)'vartrend',(Vartrn(i),i=1,3),sig
       END IF
       IF(.not.dpeq(Varsad(1),DNOTST))THEN
        sig=' '
        IF(Varsad(1).le.Varsad(2))sig='*'
        IF(Svltab(LSLCVR))
     &     WRITE(Ng,1040)'    Sadj Variance',(Varsad(i),i=1,3),sig
        IF(Lsumm.gt.0)
     &     WRITE(Nform,1040)'varseasadj',(Varsad(i),i=1,3),sig
       END IF
       IF(.not.dpeq(Varirr(1),DNOTST))THEN
        sig=' '
        IF(Varirr(1).le.Varirr(2))sig='*'
        IF(Svltab(LSLCVR))
     &     WRITE(Ng,1040)'    Irregular Variance',(Varirr(i),i=1,3),sig
        IF(Lsumm.gt.0)WRITE(Nform,1040)'varirreg',(Varirr(i),i=1,3),sig
       END IF
       IF(.not.dpeq(Varsea(1),DNOTST))THEN
        sig=' '
        IF(Varsea(1).le.Varsea(2))sig='*'
        IF(Svltab(LSLCVR))
     &     WRITE(Ng,1040)'    Seasonal Variance',(Varsea(i),i=1,3),sig
        IF(Lsumm.gt.0)
     &     WRITE(Nform,1040)'varseasonal',(Varsea(i),i=1,3),sig
       END IF
