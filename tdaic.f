...
c-----------------------------------------------------------------------
          IF(kf2.eq.0)THEN
c         CALL addate(Begspn,Sp,-Nbcst,Begadj)
c         Nadj=Nspobs+Nbcst+max(Sp,Nfcst-Fctdrp)
...
c-----------------------------------------------------------------------
c     Copy the parameter estimates to temporary variables
c-----------------------------------------------------------------------
c        CALL copy(Arimap,PARIMA,1,tap0)
c        CALL copy(B,PB,1,tb0)
       ELSE
        ...
        IF(i.eq.2)THEN
         Aicint=thisTD
         aictd=Aicc
         IF((.not.dpeq(Pvaic,DNOTST)).and.dpeq(Rgaicd(PTDAIC),ZERO))
     &       nbtd=Nb
        ELSE
         IF((.not.dpeq(Pvaic,DNOTST)).and.dpeq(Rgaicd(PTDAIC),ZERO))THEN
          aicdf=nbtd-Nb
          CALL chsppf(Pvaic,aicdf,thiscv,Mt1)
          Rgaicd(PTDAIC)=thiscv-2D0*DBLE(aicdf)
         END IF
         Dfaict=Aicc-aictd
         IF(Dfaict.le.Rgaicd(PTDAIC))THEN
          Aicint=thisTD
          aictd=Aicc
          IF((.not.dpeq(Pvaic,DNOTST)).and.dpeq(Rgaicd(PTDAIC),ZERO))
     &        nbtd=Nb
...
c     Copy the parameter estimates to temporary variables
c-----------------------------------------------------------------------
c         CALL copy(Arimap,PARIMA,1,tap)
c         CALL copy(B,PB,1,tb)
       END IF
      END DO
c-----------------------------------------------------------------------
      Dfaict=aicno-aictd
