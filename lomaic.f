...
c-----------------------------------------------------------------------
      ...
      IF((.not.dpeq(Pvaic,DNOTST)).and.dpeq(Rgaicd(PLAIC),ZERO))THEN
       CALL chsppf(Pvaic,1,thiscv,Mt1)
       Rgaicd(PLAIC)=thiscv-2D0
      END IF
      IF(Dfaicl.gt.Rgaicd(PLAIC))THEN
       IF(Lprt)WRITE(Mt1,1030)Rgaicd(PLAIC),'with',lnstr(1:nlnchr)
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
     ...
     &                Priadj,Reglom,Nrxy,Begxy,frstry,T,Elong)
       ...
       END IF
      ELSE
       IF(Lprt)WRITE(Mt1,1030)Rgaicd(PLAIC),'without',lnstr(1:nlnchr)
       IF(klm.gt.0)THEN
        ilom=1
        DO WHILE (ilom.gt.0)
         ilom=strinx(T,Colttl,Colptr,1,Ncoltl,'Length-of-')
