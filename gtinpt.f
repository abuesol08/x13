C     Last change:  SRD  25 Jan 100    2:09 pm
      SUBROUTINE gtinpt(Sscut,Srsttl,Nsrscr,Ttlvec,Notc,Lx11,X11agr,
     &                  Lseats,Lmodel,Ldata,Dtafil,Hvmfil,Mdlfil,Inptok)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c-----------------------------------------------------------------------
   40   IF(Hvmfil)THEN
     ...
     &n a model file is given.')
         Inptok=F
        END IF
        CALL getreg(Begsrs,Nobs,havsrs,havesp,Userx,Nrusrx,Bgusrx,
     &              Itdtst,Leastr,Eastst,Luser,Elong,Adjtd,Adjao,Adjls,
     &              Adjtc,Adjso,Adjhol,Adjsea,Adjcyc,Adjusr,Nusrrg,
     &              havtca,Rgaicd,Lam,Fcntyp,havhol,Lomtst,Ch2tst,
     &              Chi2cv,Tlimit,Pvaic,Inptok)
        IF(Lfatal)RETURN
        IF(.not.Lmodel)Lmodel=T
...
c-----------------------------------------------------------------------
   90   CALL getchk(Mxcklg,Acflim,Iqtype,Sp,Inptok)
        IF(Lfatal)RETURN
        Ldestm=T
        IF(.not.Lmodel)Lmodel=T
        GO TO 210
...
c-----------------------------------------------------------------------
  110   CALL getx11(havesp,Sp,Muladd,Kfulsm,Sigml,Sigmu,Lterm,Ktcopt,
...
c    Seats spec
c-----------------------------------------------------------------------
  160   CALL gtseat(Qmax2,Out2,Maxit2,Epsph2,Xl2,Rmod2,Epsiv2,Hplan2,
     &              Lseats,Lnoadm,Kmean,Lhpc,Lstsea,Bias2,Lfinit,Iphtrf,
     &              Tabtbl,Inptok)
        IF(Lfatal)RETURN
        Ldestm=T
        IF(.not.Lmodel)Lmodel=T
...
c-----------------------------------------------------------------------
  200   CALL gtspec(Sp,Begspn,Endspn,Havesp,Bgspec,Spcdff,Spctyp,Spcsrs,
     ...
     &              Plocal,Spdfor,Lstdff,Lprsfq,Ltk120,Llogqs,Inptok)
        IF(Lfatal)RETURN
        IF(Inptok)hvspec=T
        GO TO 210
       END IF
...
c-----------------------------------------------------------------------
       IF(Hvmfil)THEN
        IF(Inptok)THEN
         CALL gtmdfl(Mdlfil,Mtm,Begsrs,Nobs,havsrs,havesp,Userx,Nrusrx,
     &               Bgusrx,Itdtst,Lmodel,Lestim,havreg,Leastr,Eastst,
     &               Luser,Elong,havtca,havhol,Rgaicd,Lam,Fcntyp,Lomtst,
...
c-----------------------------------------------------------------------
       IF(Priadj.gt.1.and.Nb.eq.0)THEN
        DO icol=1,Nb
         IF(Rgvrtp(icol).eq.PRGTLM.or.Rgvrtp(icol).eq.PRGTLQ.or.
     &      Rgvrtp(icol).eq.PRGTLY.or.Rgvrtp(icol).eq.PRGTSL)THEN
          WRITE(STDERR,1020)
          WRITE(Mt2,1020)
          Inptok=F
          GO TO 220
