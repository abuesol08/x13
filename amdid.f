...
c     ------------------------------------------------------------------
     ...
     &                 bmax,dbic,xmu,nltbak
      ...
      INTEGER bstrar,bstrdf,bstrma,bstsar,bstsdf,bstsma,frstry,i,
...
c-----------------------------------------------------------------------
      IF(.not.Convrg)THEN
       WRITE(Mt1,1060)Tol
       WRITE(Mt2,1060)Tol
 1060  FORMAT(/,' ERROR: Estimation failed to converge for ',
     &          'automatically identified model',
     &        /,'        when convergence tolerence reset to ',e13.6)
       WRITE(Mt1,1070)MDLSEC,PRGNAM,DOCNAM
       WRITE(Mt2,1070)MDLSEC,PRGNAM,DOCNAM
 1070  FORMAT(/,'        Rerun program trying one of the following:',/,
...
c-----------------------------------------------------------------------
      Bstdsn=Mdldsn
      Nbstds=Nmddcr
      IF(Prttab(LAUB5M).or.Svltab(LSLB5M))THEN
