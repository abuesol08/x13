C     Last change:  BCM  14 May 1998    9:17 am
      SUBROUTINE fcnar(Na,Testpm,Estprm,A,Lauto,Err,Lckinv)
      IMPLICIT NONE
c-----------------------------------------------------------------------
...
c     ------------------------------------------------------------------
      ...
      LOGICAL Lckinv,Lauto
      INTEGER Err,info,Na,ntmpcr,Testpm
      ...
      IF(info.ne.0)THEN
       IF(Lprier)THEN
        IF(info.eq.PINVER)THEN
         ...
         IF(Lfatal)RETURN
         IF(.not.Lauto)WRITE(Mt1,1010)tmpttl(1:ntmpcr),cdot
         CALL errhdr
         WRITE(Mt2,1010)tmpttl(1:ntmpcr),cdot
c     ------------------------------------------------------------------
        ELSE IF(info.eq.PGPGER)THEN
         IF(.not.Lauto)WRITE(Mt1,1020)PRGNAM,cdot
         CALL errhdr
         WRITE(Mt2,1020)PRGNAM,cdot
c     ------------------------------------------------------------------
        ELSE IF(info.eq.PACFER)THEN
         IF(.not.Lauto)WRITE(Mt1,1030)cdot
         CALL errhdr
         WRITE(Mt2,1030)cdot
c     ------------------------------------------------------------------
        ELSE IF(info.eq.PVWPER)THEN
         IF(.not.Lauto)WRITE(Mt1,1040)cdot
         CALL errhdr
         WRITE(Mt2,1040)cdot
        END IF
c     ------------------------------------------------------------------
