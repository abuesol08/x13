...
c-----------------------------------------------------------------------
        ...
        END IF
        IF(Rgvrtp(i1).eq.PRGTAA.or.Rgvrtp(i1).eq.PRGTAO)iao=iao+1
        IF(Rgvrtp(i1).eq.PRGTAL.or.Rgvrtp(i1).eq.PRGTLS)ils=ils+1
        IF(Rgvrtp(i1).eq.PRGTAT.or.Rgvrtp(i1).eq.PRGTTC)itc=itc+1
*        IF(Rgvrtp(i1).eq.PRGTAS.or.Rgvrtp(i1).eq.PRGTSO)iso=iso+1
        IF(Rgvrtp(i1).eq.PRGTSO)iso=iso+1
        IF(Rgvrtp(i1).eq.PRGTRP.or.Rgvrtp(i1).eq.PRGTQD.or.
     &     Rgvrtp(i1).eq.PRGTQI)iramp=iramp+1
        IF(Rgvrtp(i1).eq.PRGTTL)itls=itls+1
       END DO
      END IF
      IF(Ncusrx.gt.0)THEN
       DO i1=1,Nb
        IF(Usrtyp(i1).eq.PRGTAO.or.Usrtyp(i1).eq.PRGTLS.or.
     &     Usrtyp(i1).eq.PRGTTC.or.Usrtyp(i1).eq.PRGTRP.or.
     &     Usrtyp(i1).eq.PRGTSO)iuser=iuser+1
       END DO
      END IF
      IF(Lsumm.gt.0)THEN
       ...
       WRITE(Nform,1080)'outlier.total: ',
     &                  iao+ils+itc+iramp+iuser+iso+itls
