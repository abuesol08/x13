...
c-----------------------------------------------------------------------
      ...
      PARAMETER(LCLOSE=T,ONE=1D0,PI=3.141592654D0,TWO=2D0,ZERO=0D0,
     &          MONE=-1D0)
c-----------------------------------------------------------------------
      INCLUDE 'stdio.i'
      INCLUDE 'srslen.prm'
...
c-----------------------------------------------------------------------
       IF(lnxstp)THEN
        ...
        CALL lmdif(fcnar,Na,Nestpm,estprm,A,Lauto,tnltol,ZERO,ZERO,
     &             Nliter+tnlitr,eps,diag,1,100D0,nprint,info,Nliter,
     &             Nfev,Armacm,PA,ipvt,qtf,wa1,wa2,wa3,wa4)
        IF(Lfatal)RETURN
...
c        CALL fcnar(Na,tmpa,estprm,tmpa,Lauto,info,F)
        CALL fcnar(Na,Nestpm,estprm,tmpa,Lauto,info,F)
        IF(Lfatal)RETURN
c        write(ng,9000)'after fcnar',iter, lnxstp, info, convrg, armaer
        iflag=2
        CALL fdjac2(fcnar,Na,Nestpm,estprm,tmpa,Lauto,Armacm,PA,iflag,
     &              0d0,wa4,F)
        CALL upespm(estprm)
c-----------------------------------------------------------------------
        IF(iflag.ge.0)THEN
