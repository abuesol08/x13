...
c-----------------------------------------------------------------------
      ...
      PARAMETER(USXDIC=
     &   'tdtdstockeasterlaborthanksaoholidaysceasteruser')
c-----------------------------------------------------------------------
      CHARACTER MTDDIC*12
      INTEGER mtdptr,PMTD
      PARAMETER(PMTD=2)
...
c-----------------------------------------------------------------------
     ...
     &            223,231,244,259,264,272/
      DATA usxptr/1,3,10,16,21,27,29,36,44,48/
      DATA mtdptr/1,7,13/
      DATA napptr/1,3,10/
...
c-----------------------------------------------------------------------
   10   CALL gtpdrg(Begsrs,Nobs,Havsrs,Havesp,T,Havxtd,Havxhl,havln,
     &              havlp,argok,Inptok)
        IF(Lfatal)RETURN
        ...
        IF(argok.and.Nusxrg.gt.0)THEN
         DO i=1,Nusxrg
          ...
          ELSE IF(usxidx(i).eq.3)THEN
           Usxtyp(i)=PRGTEA
           Havxhl=T
          ELSE IF(usxidx(i).eq.4)THEN
           Usxtyp(i)=PRGTLD
           Havxhl=T
          ELSE IF(usxidx(i).eq.5)THEN
           Usxtyp(i)=PRGTTH
           Havxhl=T
          ELSE IF(usxidx(i).eq.6)THEN
           Usxtyp(i)=PRGTAO
          ELSE IF(usxidx(i).eq.7)THEN
           Usxtyp(i)=PRGTUH
           Havxhl=T
          ELSE IF(usxidx(i).eq.8)THEN
           Usxtyp(i)=PRGTEC
           Havxhl=T
          ELSE IF(usxidx(i).eq.9.or.usxidx(i).eq.NOTSET)THEN
           Usxtyp(i)=PRGTUD
          END IF
         END DO
        END IF
